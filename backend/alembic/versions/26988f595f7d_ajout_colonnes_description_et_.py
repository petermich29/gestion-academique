"""Ajout colonnes description et abbreviation a mention historique

Revision ID: 26988f595f7d
Revises: 0c30b6abdc19
Create Date: 2025-12-05 13:46:10.971878
...
revision: str = '26988f595f7d'
down_revision: Union[str, Sequence[str], None] = '0c30b6abdc19'
...
def upgrade() -> None:
    ...
    op.add_column('mentions_historique', sa.Column('Mention_description_historique', sa.Text(), nullable=True))
    op.add_column('mentions_historique', sa.Column('Mention_abbreviation_historique', sa.String(length=20), nullable=True))
...
def downgrade() -> None:
    ...
    sa.ForeignKeyConstraint(['UE_id_fk'], ['unites_enseignement.UE_id'], name='elements_constitutifs_UE_id_fk_fkey'),
    sa.PrimaryKeyConstraint('EC_id', name='elements_constitutifs_pkey'),
    sa.UniqueConstraint('EC_code', 'UE_id_fk', name='uq_ec_code_ue', postgresql_include=[], postgresql_nulls_not_distinct=False),
    postgresql_ignore_search_path=False
    )
    ...
    sa.PrimaryKeyConstraint('TypeComposante_id', name='types_composante_pkey'),
    sa.UniqueConstraint('TypeComposante_label', name='types_composante_TypeComposante_label_key', postgresql_include=[], postgresql_nulls_not_distinct=False),
    postgresql_ignore_search_path=False
    )
    ...
    sa.Column('Institution_id_fk', sa.VARCHAR(length=10), autoincrement=False, nullable=False),
    sa.Column('Composante_type', sa.VARCHAR(length=7), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['Composante_type'], ['types_composante.TypeComposante_id'], name='composantes_TypeComposante_id_fk_fkey'),
