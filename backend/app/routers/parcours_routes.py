...
def get_parcours_structure(parcours_id: str, db: Session = Depends(get_db)):
    # (Code existant conservé pour la structure académique...)
    try: db.expire_all() 
    except: pass
        ...
        for sem in sorted(niveau.semestres, key=lambda x: x.Semestre_numero):
            ...
            for ue in sorted(ues, key=lambda x: x.UE_code):
                ues_data.append(schemas.StructureUE(
                    id=ue.UE_id, code=ue.UE_code, intitule=ue.UE_intitule, 
                    credit=ue.UE_credit, ec_count=len(ue.elements_constitutifs)
                ))
            semestres_data.append(schemas.StructureSemestre(id=sem.Semestre_id, numero=sem.Semestre_numero, code=sem.Semestre_code, ues=ues_data))
        structure_response.append(schemas.StructureNiveau(niveau_id=niveau.Niveau_id, niveau_label=niveau.Niveau_label, semestres=semestres_data))
